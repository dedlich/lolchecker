...
class GameContext:
    ...
    enemy_cc_count: int = 0      # hard-CC kits (stun / suppress / unbounded knockup)
    enemy_burst_count: int = 0   # single-target burst threats (assassins + burst mages)
...
@dataclass(frozen=True)
class BuildSwap:
    """An item in the standard build that the current matchup demoted —
    paired with a higher-scored alternative the user should buy instead.

    Generated by ``compute_swap_suggestions`` which scores items twice
    (neutral baseline vs ``GameContext``) and diffs the top-N rankings.
    Surfaced in-game by ``rule_build_swap``.
    """
    skip_item: str          # item name in the standard build to deprioritize
    skip_item_id: int       # numeric id (so the rule can render an icon)
    replacement: str        # alternative that scored higher in this matchup
    replacement_id: int     # numeric id of the replacement
    reason: str             # short why ("vs 3 AP-Gegner — MR fits better")
    score_delta: float      # how much better the replacement scored
...
class BuildResult:
    ...
    starter_id: int | None
    swap_suggestions: tuple[BuildSwap, ...] = ()  # items to skip + replacements
...
def score_item(
    ...
    if context is not None and score > 0:
        ...
        # Heavy CC enemy comp → tenacity / cleanse-family items.
        # Quicksilver Sash + Mercurial Scimitar break the kill-via-CC
        # pattern; Mikael's Blessing + Banshee's Veil mitigate it.
        # Sterak's tenacity passive is also worth boosting on bruisers.
        if context.enemy_cc_count >= 2 and kw(
            "quicksilver", "mercurial", "mikael",
            "banshee", "sterak", "edge of night",
        ):
            bonus = 25 + context.enemy_cc_count * 8
            score += bonus
            reasons.append(
                f"+{bonus:.0f} vs {context.enemy_cc_count} CC-Gegner (Tenacity/Cleanse)"
            )

        # Burst-heavy enemy comp → stasis / shield / spell-block items.
        # Zhonya's + Stopwatch + GA + Hexdrinker / Maw / Banshee's are
        # the canonical "don't die to one combo" items.
        if context.enemy_burst_count >= 1 and kw(
            "stasis", "zhonya", "stopwatch", "guardian angel",
            "hexdrinker", "maw of malmortius", "edge of night",
            "banshee", "shieldbow", "immortal",
        ):
            bonus = 20 + context.enemy_burst_count * 10
            score += bonus
            reasons.append(
                f"+{bonus:.0f} vs {context.enemy_burst_count} Burst-Gegner (Anti-Burst)"
            )

        # Mobility-heavy enemy comp → slow auras + Randuin's-style
        # crit-mitigation. Frozen Heart's mana drain + AS slow neuters
        # auto-attack mobility kits (Yasuo, Tryndamere); Iceborn's
        # zone slow does the same on bruisers.
        if context.enemy_mobility_count >= 2 and kw(
            "frozen heart", "iceborn", "randuin",
            "dead man", "thornmail", "kaenic",
        ):
            bonus = 15 + context.enemy_mobility_count * 6
            score += bonus
            reasons.append(
                f"+{bonus:.0f} vs {context.enemy_mobility_count} Mobility-Gegner (Slow-Aura)"
            )
...
def recommend_items(
    ...
    swap_suggestions = compute_swap_suggestions(
        ...
        items=items,
        archetype=archetype,
        context=context if context is not None else GameContext(),
        scaling=scaling,
    )
...
# Score gap that justifies suggesting a swap. Below this the standard
# build's choice is "good enough" and we don't badger the user.
_SWAP_MIN_DELTA = 25.0
# Top-N rank to consider for the standard "core" build that swap diffs
# against. Boots + starter slots are excluded from this comparison.
_SWAP_CORE_DEPTH = 6
# How deep to look in the contextual ranking for a replacement. We
# prefer items that ALREADY show up among the top-12 with context.
_SWAP_REPLACEMENT_DEPTH = 12
# Cap how many suggestions a single BuildResult carries — surface up
# to 3 so the in-game recommendation panel doesn't drown in swaps.
_SWAP_MAX_SUGGESTIONS = 3


def compute_swap_suggestions(
    *,
    champion: dict,
    items: dict,
    archetype: ChampionArchetype,
    ...
    """Diff the neutral-baseline ranking against the matchup-aware
    ranking. Items in the neutral top-6 that drop substantially under
    the live context are flagged as "skip", paired with a higher-scoring
    replacement that wasn't already in the neutral top-6.

    Returns a tuple of up to ``_SWAP_MAX_SUGGESTIONS`` BuildSwaps,
    sorted by ``score_delta`` descending so the most-impactful swap
    surfaces first. Returns ``()`` when there's nothing material to
    suggest — empty matchup, no measurable score gaps, etc.

    Pure function: makes its own internal score lists. ``recommend_items``
    calls this and stamps the result onto ``BuildResult.swap_suggestions``.
    """
    if not _has_meaningful_context(context):
        # No enemy data yet → no swap is computable. Return empty so
        # the rule stays silent rather than emitting noise.
        return ()

    neutral_ctx = GameContext(game_time_s=context.game_time_s)
    neutral_ranking = _ranked_completed_items(
        champion, items, archetype, neutral_ctx, scaling,
    )
    contextual_ranking = _ranked_completed_items(
        champion, items, archetype, context, scaling,
    )
    if not neutral_ranking or not contextual_ranking:
        return ()

    neutral_top = neutral_ranking[:_SWAP_CORE_DEPTH]
    neutral_top_ids = {s.item_id for s in neutral_top}
    contextual_top_ids = {
        s.item_id for s in contextual_ranking[:_SWAP_CORE_DEPTH]
    }
    # Lookup contextual scores by id so we can compute the per-item delta.
    contextual_by_id = {s.item_id: s for s in contextual_ranking}

    swaps: list[BuildSwap] = []
    for neutral_item in neutral_top:
        # Item is still in the contextual top-6 → no swap needed.
        if neutral_item.item_id in contextual_top_ids:
            continue
        contextual_score = contextual_by_id.get(neutral_item.item_id)
        if contextual_score is None:
            # Item dropped off the ranked list entirely (filtered out
            # at the score≤0 cutoff). That IS a swap candidate.
            current_score = 0.0
        else:
            current_score = contextual_score.score
        # Find the best contextual item that wasn't in the neutral top —
        # i.e. the addition the matchup justifies that the standard
        # build doesn't already cover.
        replacement = next(
            (
                s for s in contextual_ranking[:_SWAP_REPLACEMENT_DEPTH]
                if s.item_id not in neutral_top_ids
            ),
            None,
        )
        if replacement is None:
            continue
        delta = replacement.score - current_score
        if delta < _SWAP_MIN_DELTA:
            continue
        reason = _swap_reason(replacement)
        swaps.append(BuildSwap(
            skip_item=neutral_item.item_name,
            skip_item_id=neutral_item.item_id,
            replacement=replacement.item_name,
    ...
    return tuple(swaps[:_SWAP_MAX_SUGGESTIONS])


def _has_meaningful_context(context: GameContext) -> bool:
    """Whether the matchup context carries any signal at all. With
    no enemy data populated, swap recommendations would be noise."""
    return bool(
        context.enemy_ap_count
        or context.enemy_ad_count
        or context.enemy_sustain_count
        or context.enemy_tank_count
        or context.enemy_cc_count
        or context.enemy_burst_count
        or context.enemy_mobility_count
        or context.player_behind
    )


def _ranked_completed_items(
    champion: dict,
    items: dict,
    archetype: ChampionArchetype,
    context: GameContext,
    scaling: ChampionScalingProfile | None,
) -> list[ScoredItem]:
    """Score every legal completed item under ``context`` and return the
    ranked list. Same filtering as ``recommend_items``'s internal pass —
    extracted so swap-suggestion can reuse it without duplicating the
    boots / starter / synergy plumbing."""
    all_items = [
        ({**v, "id": int(k)} if not int(v.get("id") or 0) and k.isdigit() else v)
        for k, v in items.items()
    ]
    ...
    scored = [
        score_item(item, archetype, context, scaling) for item in completed
    ]
    return sorted(
        (s for s in scored if s.score > 0),
        key=lambda s: s.score,
...
def _swap_reason(replacement: ScoredItem) -> str:
    """Pick the single most-actionable line out of the replacement's
    score reasons. Prefers context-tagged reasons (the ones that
    mention "Gegner" / "vs") since those explain why this matchup
    surfaces the replacement; falls back to the first reason
    otherwise."""
    for r in replacement.reasons:
        if "Gegner" in r or " vs " in r.lower() or "Sustain" in r:
            return r.lstrip("+ ").strip()
    if replacement.reasons:
        return replacement.reasons[0].lstrip("+ ").strip()
    return f"höher gescoret in dieser Runde (+{replacement.score:.0f})"
